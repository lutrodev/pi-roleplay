...
export function ReadingControls({ value, onChange }: {
  ...
  return <div className="reading-workbench"><div className="reading-controls-layout">
    <section className="reading-preview" data-preview-theme={theme} style={{ ...dialogueColorStyle(value.dialogueColor), ...italicColorStyle(value.italicColor, value.italicHighlight) }}><header><strong>{uiT('正文预览')}</strong><small>{uiT('保存后应用到正文')}</small></header><div className="reading-sample" style={{ '--reading-font': value.fontFamily === 'serif' ? 'var(--serif)' : 'var(--font-sans)', '--reading-size': `${value.fontSize}px`, '--reading-leading': value.lineHeight, '--reading-width': `${value.maxWidth}px` } as CSSProperties}><Markdown text={`${uiT('黄昏时，灯塔的门开了。')}\n\n${uiT('“你终于来了。”她说。')}\n\n*${uiT('她在心里松了一口气。')}*`} highlight={highlight} /></div></section>
    <div className="reading-fields stack">
    ...
    <details className="reading-highlights"><summary>{uiT('文字高亮')}<small>{uiT('对白与斜体的颜色和显示方式')}</small></summary><div className="stack">
