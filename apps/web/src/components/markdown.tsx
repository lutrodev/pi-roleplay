...
export const Markdown = memo(function Markdown({ text, highlight = false, search = '' }: { text: string; highlight?: boolean; search?: string }) {
  return <div className="prose"><ReactMarkdown remarkPlugins={highlight ? [remarkGfm, dialoguePlugin] : [remarkGfm]} rehypePlugins={search ? [[searchPlugin, { query: search }]] : []} skipHtml components={{
