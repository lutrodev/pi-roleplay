...
import { Button, Empty, ErrorNotice, Field, Input, Loading, Modal, Select } from './ui.tsx'

export function ModelPicker({ value, onChange, inheritLabel = uiT('跟随系统默认模型'), allowInherit = true, inheritDetail, defaultChoice, label = uiT('模型'), layout = 'row' }: {
  ...
  return <div className="model-picker">{layout === 'compact' ? trigger : <SettingRow kind="field" label={label}>{trigger}</SettingRow>}
    {!defaultChoice?.selected && selected?.thinkingLevels.some(level => level !== 'off') && <Field layout="row" label={uiT('思考强度')}><Select aria-label={uiT('%{label}思考强度', { label })} value={value?.reasoningEffort ?? 'off'} onChange={event => onChange({ ...value!, reasoningEffort: event.target.value })}>{selected.thinkingLevels.map(level => <option key={level} value={level}>{level === 'off' ? uiT('关闭', { context: 'reasoning' }) : level}</option>)}</Select></Field>}
    <ErrorNotice error={query.error} retry={() => void query.refetch()} retrying={query.isFetching} />
    <Modal size="form" className="settings-modal model-picker-dialog" open={open} onOpenChange={setOpen} title={uiT('选择模型')}><div className="stack"><div className="search-field"><Search size={16} /><Input autoFocus aria-label={uiT('搜索可用模型')} placeholder={uiT('搜索模型名称、ID 或连接')} value={search} onChange={event => setSearch(event.target.value)} /></div><ErrorNotice error={query.error} retry={() => void query.refetch()} retrying={query.isFetching} />{query.isPending && <Loading />}
