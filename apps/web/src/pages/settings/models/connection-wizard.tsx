...
export function ConnectionWizard({ revision, provider, done, onDefault }: { revision: number; provider?: Provider; done: () => void; onDefault: (route: ModelRoute | null) => Promise<void> }) {
  ...
  const [truncated, setTruncated] = useState(false), [discoveryId, setDiscoveryId] = useState<string>(), autoFetched = useRef('')
  ...
  const dirty = !finished && (!!apiKey || !!manual || selected.length > 0 || !provider && (!!label || step > 0))
  function resetDiscovery() { setAvailable(null); setDiscoveryId(undefined); setSelected([]); setSearch(''); setTruncated(false); discover.clear(); save.clear() }
  ...
  async function fetchModels() {
    await discover.run(async () => { const result = await api<Discovery>('/settings/providers/discover', 'POST', { connection, ...(provider ? { provider: provider.id } : {}), ...(apiKey.trim() ? { apiKey: apiKey.trim() } : {}) }); setAvailable(result.models); setDiscoveryId(result.discoveryId); setTruncated(result.truncated ?? false); setSelected(current => current.filter(id => result.models.some(model => model.model === id))) })
  }
  const discoveryIdentity = JSON.stringify([connection, apiKey, provider?.id])
  useEffect(() => {
    if (step === 2 && !finished && (apiKey.trim() || provider?.credentialConfigured) && autoFetched.current !== discoveryIdentity) {
      autoFetched.current = discoveryIdentity; void fetchModels()
    }
  }, [step, finished, discoveryIdentity])
  ...
  return <EditorForm className="stack model-editor connection-wizard" data-step={step} dirty={dirty} busy={save.busy || discover.busy} onSubmit={event => { event.preventDefault(); if (step === 1) { if (!apiKey.trim() && !provider?.credentialConfigured) setMode('manual'); setStep(2); return } if (step !== 2) return; void save.run(async () => {
    ...
    const added = ids.map(model => freshModel(connection, model))
    const result = await api<Catalog>('/settings/providers', provider ? 'PUT' : 'POST', { expectedRevision: revision, provider: { id: provider?.id ?? id, label, connection, discoveryId, models: [...(provider?.models.map(fields) ?? []), ...added], ...(apiKey.trim() ? { apiKey: apiKey.trim() } : {}) } })
    ...
    {step === 2 && <><div className="connection-caption">{uiT('服务连接')}<strong>{label}</strong></div><div className="model-source-switch"><Button aria-pressed={mode === 'list'} onClick={() => setMode('list')}>{uiT('从服务获取')}</Button><Button aria-pressed={mode === 'manual'} onClick={() => setMode('manual')}>{uiT('手动填写 ID')}</Button></div>
      ...
      <p className="muted">{uiT('自动识别名称、图片、思考和长度规格；未识别的项目可在添加后核对。')}</p>
    </>}
...
function ConnectionComplete({ catalog, providerId, ids, done, onDefault }: { catalog: Catalog; providerId: string; ids: string[]; done: () => void; onDefault: (route: ModelRoute | null) => Promise<void> }) {
  ...
  return <div className="stack connection-complete"><div className="completion-mark"><CheckIcon size={24} /></div><h3>{uiT('已添加 %{count} 个模型', { count: ids.length })}</h3><p className="muted">{uiT('可以现在测试并设为默认，也可以稍后在模型列表中操作。')}</p><Field label={uiT('模型')}><Select value={selected} onChange={event => { setSelected(event.target.value); setResult(null); setDefaultSaved(false) }}>{ids.map(id => <option key={id} value={id}>{provider.models.find(model => model.model === id)?.effective.label || id}</option>)}</Select></Field>
    {!provider.credentialConfigured && <StatusNotice compact tone="warning" title={uiT('缺少密钥')} details={uiT('尚未填写密钥。请在服务连接中补全后测试。')} />}
    {result && <StatusNotice compact tone={result.status === 'failed' ? 'error' : 'success'} title={uiT(result.status === 'failed' ? '连接测试未通过' : '连接测试通过')} details={uiT(result.message)} />}{defaultSaved && <p className="connection-default-saved" role="status"><CheckIcon size={16} aria-hidden="true" />{uiT('已设为默认模型')}</p>}<ErrorNotice error={action.error} />
