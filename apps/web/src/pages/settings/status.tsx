...
export function SystemStatus({ onNavigate }: { onNavigate: (tab: 'models' | 'tools') => void }) {
  const query = useQuery({ queryKey: ['system-status'], queryFn: () => api<{ version: string; operations: { quiesced: boolean; backup: { status: string; completedAt?: string; file?: string } | null }; tools: { available: boolean; active?: number }; models: { label: string; configured: boolean }[]; searchConfigured: boolean; concurrency: { active: number; queued: number }; runs: { status: string; count: number }[] }>('/system/status'), refetchInterval: 15000 })
  return <section className="settings-form stack">
    ...
    {query.data && <SettingsGroup>
      ...
      <SettingRow label={uiT('模型请求')}><span className="setting-status">{uiT('%{active} 个进行中 · %{queued} 个排队', query.data.concurrency)}</span></SettingRow>
