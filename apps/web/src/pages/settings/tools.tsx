...
function ToolSettingsEditor({ initial, onDirtyChange }: { initial: ToolSettingsSnapshot; onDirtyChange?: (dirty: boolean) => void }) {
  ...
  return <><EditorForm className="stack" dirty={dirty} busy={action.busy} onSubmit={event => { event.preventDefault(); void action.run(async () => {
    ...
    <SettingRow label={uiT('搜索密钥')} help={<span role="status">{clearKey ? uiT('保存后移除当前密钥，保存前可以撤销。') : baseline.searchKey.configured ? baseline.searchKey.source === 'saved' ? uiT('搜索密钥已加密保存') : uiT('使用部署环境中的搜索密钥') : uiT('尚未配置搜索密钥')}</span>}><Button tone="quiet" disabled={!baseline.searchKey.configured && !clearKey} onClick={() => { setClearKey(!clearKey); setSaved(false); if (!clearKey) setKey('') }}>{clearKey ? uiT('撤销移除') : uiT('移除密钥')}</Button></SettingRow>
