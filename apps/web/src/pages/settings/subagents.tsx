...
import { api, queryClient, useAction, useModels } from '../../lib/api.ts'
...
export function Subagents({ enabled, onEnabledChange }: { enabled: boolean; onEnabledChange: (enabled: boolean) => Promise<void> }) {
  useUiLanguage()
  ...
  const [search, setSearch] = useState(''), models = useModels()
  ...
  return <div className="settings-form stack subagent-catalog">
    <SettingsGroup description={uiT('应用于所有会话，开关修改后立即保存。')}><SettingToggle label={uiT('允许调用任务子代理')} help={uiT('在 Agent 模式下，主模型可调用已启用的任务子代理。关闭后保留单项配置，不影响 Writer。')} checked={enabled} disabled={action.busy} onChange={event => void action.run(() => onEnabledChange(event.target.checked))} /></SettingsGroup>
    ...
    <ErrorNotice source="read" error={query.error} /><EditorFooter error={remove ? null : action.error} />
