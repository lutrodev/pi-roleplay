...
export function Subagents() {
  ...
  return <div className="settings-form stack">
    <p className="muted">{uiT('这里设置所有会话的默认子代理。可在会话顶部的“子代理”中单独调整模型。')}</p>
...
function WriterEditor({ writer, done }: { writer: SubagentCatalog['writer']; done: () => void }) {
  ...
  return <EditorForm className="stack" dirty={JSON.stringify(route) !== JSON.stringify(writer.route)} busy={action.busy} onSubmit={event => { event.preventDefault(); void action.run(async () => { await api('/settings/writer', 'PUT', { expectedRevision: writer.revision, route }); done() }) }}><SettingsGroup><ModelPicker value={route.kind === 'fixed' ? route : null} inheritLabel={uiT("跟随主模型")} inheritDetail={uiT("每次调用跟随所在会话的主模型")} inheritedReasoning={{ value: route.reasoningEffort, onChange: reasoningEffort => setRoute({ kind: 'inherit', reasoningEffort }) }} onChange={next => setRoute(next ? { kind: 'fixed', ...next } : { kind: 'inherit' })} /></SettingsGroup><ErrorNotice error={action.error} /><div className="form-actions sticky-actions"><CancelButton onCancel={done} /><Button type="submit" tone="primary" disabled={action.busy || JSON.stringify(route) === JSON.stringify(writer.route)}>{uiT("保存 Writer 设置")}</Button></div></EditorForm>
}
function TaskEditor({ agent, done }: { agent?: TaskSubagent; done: () => void }) {
  ...
  return <EditorForm className="stack" dirty={JSON.stringify(value) !== baseline} busy={action.busy} onSubmit={event => { event.preventDefault(); void action.run(async () => { await api(agent ? `/settings/subagents/${agent.id}` : '/settings/subagents', agent ? 'PUT' : 'POST', { ...(agent ? { expectedRevision: agent.revision } : {}), subagent: value }); done() }) }}><SettingsGroup><Field layout="row" label={uiT("名称")}><Input autoFocus required maxLength={80} value={value.name} onChange={event => setValue({ ...value, name: event.target.value })} /></Field><SettingToggle label={uiT("启用这个子代理")} checked={value.enabled} onChange={event => setValue({ ...value, enabled: event.target.checked })} /></SettingsGroup><Field label={uiT("调用说明")} help={uiT("告诉主模型在什么情况下使用它，以及与 Writer 或其他子代理的关系。")}><Textarea required maxLength={240} rows={2} value={value.description} onChange={event => setValue({ ...value, description: event.target.value })} /></Field><Field label={uiT("工作指令")}><Textarea required maxLength={20000} rows={6} value={value.instructions} onChange={event => setValue({ ...value, instructions: event.target.value })} /></Field><SettingsGroup layout="form" title={uiT("模型与工具")}><ModelPicker value={value.route.kind === 'fixed' ? value.route : null} inheritLabel={uiT("跟随主模型")} inheritDetail={uiT("每次调用跟随所在会话的主模型")} inheritedReasoning={{ value: value.route.reasoningEffort, onChange: reasoningEffort => setValue({ ...value, route: { kind: 'inherit', reasoningEffort } }) }} onChange={route => setValue({ ...value, route: route ? { kind: 'fixed', ...route } : { kind: 'inherit' } })} /><p className="muted">{uiT("任务子代理默认可读取工作文件和本轮附件。")}</p><div>{(['web_search', 'skill'] as const).map(tool => <SettingToggle key={tool} label={tool === 'web_search' ? uiT("允许网页搜索") : uiT("允许加载 Skills")} checked={value.tools.includes(tool)} onChange={event => setValue({ ...value, tools: event.target.checked ? [...value.tools, tool] : value.tools.filter(item => item !== tool) })} />)}</div></SettingsGroup><ErrorNotice error={action.error} /><div className="form-actions sticky-actions"><CancelButton onCancel={done} /><Button tone="primary" type="submit" disabled={action.busy || !!agent && JSON.stringify(value) === baseline}>{uiT("保存子代理")}</Button></div></EditorForm>
}
