...
export function AssetEditorDialog({ id, kind, onClose, returnFocus }: { id: string | null; kind: AssetKind; onClose: () => void; returnFocus?: RefObject<HTMLElement | null> }) {
  useUiLanguage()
  const query = useAsset(id && id !== 'new' ? id : undefined)
  // Existing records own their type; a stale or manually edited link cannot select the wrong fields.
  const actualKind = query.data?.asset.kind ?? kind
  return <Modal open={id !== null} onOpenChange={open => { if (!open) onClose() }} title={uiT(`${id === 'new' ? '创建' : '编辑'}${assetLabels[actualKind]}`)} size="editor" className={actualKind === 'persona' ? 'persona-editor-modal' : undefined} returnFocus={returnFocus}>{id && <>
    <ErrorNotice source="read" error={query.error} retry={() => void query.refetch()} retrying={query.isFetching} />
    {id !== 'new' && query.isPending ? <Loading /> : (id === 'new' || query.data) && <AssetEditor key={`${actualKind}:${id}`} asset={query.data?.asset} kind={actualKind} done={onClose} />}
  </>}</Modal>
}
function AssetEditor({ asset, kind, done }: { asset?: AssetRecord; kind: AssetKind; done: () => void }) {
  ...
  return <EditorForm className="asset-editor stack" dirty={dirty} busy={action.busy} onSubmit={event => { event.preventDefault(); void action.run(async () => {
    ...
    <EditorFooter error={action.error}><span className="settings-save-status" role="status">{dirty ? uiT("有未保存的更改") : ''}</span><CancelButton onCancel={done} /><Button tone="primary" type="submit" disabled={action.busy}>{action.busy ? uiT("正在保存…") : uiT("保存资料")}</Button></EditorFooter>
  </EditorForm>
}
