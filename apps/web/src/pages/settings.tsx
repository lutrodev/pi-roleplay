...
function SettingsForm({ initial, onDirtyChange }: { initial: Settings; onDirtyChange: (dirty: boolean) => void }) {
  ...
  return <><TabGroup value={tab} onChange={next => { if (!action.busy) setTab(next) }} contentRef={contentRef} label={uiT("设置分类")} className="settings-layout" orientation={mobile ? 'horizontal' : 'vertical'} items={categories}>
