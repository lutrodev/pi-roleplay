...
export function FilesPanel({ storyId }: { storyId: string }) {
  ...
  return <div ref={root} className="files-reader">
    {selected ? <>
      ...
      <ErrorNotice source="read" error={preview.error} retry={() => void preview.refetch()} retrying={preview.isFetching} />{preview.isPending && <Loading />}
      ...
      <ErrorNotice source="read" error={listing.error} retry={() => void listing.refetch()} retrying={listing.isFetching} />{listing.isPending && <Loading />}
