...
export function RoundProcess({ runId, run }: { runId: string; run?: RunRecord }) {
  ...
  return <div className="round-process">{query.data && <div className="round-process-summary"><Activity size={14} /><span>{uiT('%{count} 次模型请求', { count: query.data.requests.length })} · {uiT('%{count} 次工具调用', { count: query.data.entries.filter(entry => !entry.history && entry.kind === 'tool').length })}</span></div>}<ErrorNotice title={uiT('本轮轨迹暂时无法读取')} error={query.error} retry={() => void query.refetch()} retrying={query.isFetching} /></div>
}
