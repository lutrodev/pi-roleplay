...
export function runActivityPhase(story: ActivityStory, run: RunRecord | undefined, activeTools: readonly ActiveTool[], connection: Connection = 'live', requestsQueued = false): Phase | null {
  ...
  if (active) {
    ...
    else if (run.status === 'queued' || requestsQueued) phase = 'queued'
...
export function RunActivity({ story, run, activeTools, connection = 'live', requestsQueued = false }: { story: ActivityStory; run?: RunRecord; activeTools: readonly ActiveTool[]; connection?: Connection; requestsQueued?: boolean }) {
  useUiLanguage()
  const sharedConnectionError = useSharedConnectionError()
  const phase = runActivityPhase(story, run, activeTools, connection, requestsQueued)
