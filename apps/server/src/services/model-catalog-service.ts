...
interface Catalog { version: 2; revision: number; registrations: ModelRegistration[]; labels: Record<string, string>; secrets: Record<string, string>; connections: Record<string, ModelConnection & { keyEnv: string }> }
interface StoredCheck extends ModelCheck { fingerprint: string }
export interface ProviderInput { id: string; label: string; apiKey?: string; clearKey?: boolean; connection?: ModelConnection; models: Omit<ModelRegistration, 'keyEnv' | 'provider'>[] }
...
export class ModelCatalogService {
  ...
  constructor(readonly assets: AssetRepository, initial: ModelRegistration[], sessionKey: Buffer,
    ...
    this.models = new ModelRegistry(catalog.registrations, { ...options, env: name => this.credentials.get(name) ?? (options?.env ?? (key => process.env[key]))(name) })
    if (saved === undefined || (saved as { version?: number }).version !== 2) this.save(catalog)
  }
  snapshot() {
  ...
  update(expectedRevision: number, input: unknown, create = false) {
    ...
    return this.mutate(expectedRevision, current => {
      ...
      const resolved = new ModelRegistry(parsed.models, this.options).registrations().find(model => model.provider === id)
      const defaults = connection ?? { api: resolved?.api, baseUrl: resolved!.baseUrl! }
      return { ...current, registrations: parsed.models, labels: { ...current.labels, [id]: label }, secrets,
        connections: { ...current.connections, [id]: { api: defaults.api, baseUrl: defaults.baseUrl, keyEnv: sharedKey } } }
    })
  }
  remove(expectedRevision: number, provider: string) {
    return this.mutate(expectedRevision, current => {
      ...
      return { ...current, registrations: current.registrations.filter(model => model.provider !== provider), secrets, labels, connections }
    })
  }
  private mutate(expectedRevision: number, operation: (current: Catalog) => Catalog) {
    this.assets.database.transaction(() => {
      ...
      new ModelRegistry(next.registrations, { ...this.options, env: name => credentials.get(name) ?? this.options?.env?.(name) ?? process.env[name] })
      this.save(next); this.credentials = credentials; this.models.replace(next.registrations)
    })
  ...
  private read(): Catalog {
    ...
    try {
      objectInput(value)
      requireValue((value.version === 1 || value.version === 2) && Number.isSafeInteger(value.revision) && Number(value.revision) > 0, 'MODEL_CATALOG_CORRUPT', '模型配置版本不正确。')
  ...
  private upgrade(value: Record<string, unknown>): Catalog {
    ...
    return { version: 2, revision: Number(value.revision), registrations, labels, connections, secrets: value.secrets as Record<string, string> }
  }
  ...
  private fingerprint(registration: ModelRegistration) {
    ...
    return createHash('sha256').update(JSON.stringify(configuration)).update('\0').update(this.key(registration.keyEnv) ?? '').digest('hex')
  }
  ...
  async discover(input: unknown) {
    ...
    return this.inspection(async signal => { try { return await discoverModels(connection, key, (this.options.fetch ?? fetch) as typeof fetch, signal) } catch (error) { throw inspectionError(error) } })
  }
  async test(expectedRevision: number, provider: string, model: string) {
    ...
    try {
      return await this.inspection(async signal => {
        const result = await checkModel(registration, key, this.options, signal)
        if (!this.closed) {
  ...
  close() { this.closed = true; for (const controller of this.inspections) controller.abort() }
