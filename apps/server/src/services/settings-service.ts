...
export interface SettingsSnapshot { version: 9; revision: number; preferences: Preferences }
...
export class SettingsService {
  ...
  snapshot(): SettingsSnapshot {
    return this.assets.database.transaction(() => {
      ...
      if (value === undefined) {
        const initial: SettingsSnapshot = { version: 9, revision: 1, preferences: structuredClone(DEFAULT_PREFERENCES) }
        this.save(initial); return initial
      }
      try {
        requireValue(value && typeof value === 'object' && !Array.isArray(value) && typeof value.version === 'number' && [1, 2, 3, 4, 5, 6, 7, 8, 9].includes(value.version) && Number.isSafeInteger(value.revision) && Number(value.revision) >= 1, 'INVALID_SETTINGS', '设置版本不正确。')
        if (value.version !== 9) {
          const { preferences, features } = upgradePreferences(value.preferences, value.version)
          if (features) migrateStoryFeatures(this.assets.database, features)
          const upgraded: SettingsSnapshot = { version: 9, revision: Number(value.revision) + 1, preferences }
          this.save(upgraded); return upgraded
        }
        return { version: 9, revision: Number(value.revision), preferences: normalizePreferences(value.preferences) }
  ...
  update(expectedRevision: number, input: unknown) {
    ...
    return this.assets.database.transaction(() => {
      ...
      const updated: SettingsSnapshot = { version: 9, revision: current.revision + 1, preferences }
