...
import type { CommitGeneration, GeneratedCommit, TurnService } from '../services/turn-service.ts'
...
export interface ExecutionResources {
  replyOptions?: (input: CommitGeneration, context: ReturnType<ContextService['freeze']>, journal: RunJournal) => Promise<GeneratedCommit>
...
export class RunExecutor {
  ...
  async execute(runId: string, signal: AbortSignal, resources: ExecutionResources) {
    ...
    const contextPolicy = { ...resources.contextPolicy, writerModel: routes.writer.model }
    ...
    const commit: AgentTool = {
      ...
      parameters: Type.Unsafe({ anyOf: [
        { type: 'object', additionalProperties: false, ...(mode === 'agent' ? { required: ['narrative'] } : {}), properties: {
          ...
          extensions: { type: 'object', additionalProperties: false, description: 'Omit or use {}. Enabled reply options are generated by the runtime; parent-authored extensions are not accepted.' },
      ...
      execute: async (_callId, input) => {
        const result = await toolResult(async () => {
          ...
          const event = await this.turns.commit(runId, currentOwner, finalText, commitInput, stateEnabled,
            ...
            resources.replyOptions ? input => resources.replyOptions!(input, context, journal) : undefined)
          committed = true
