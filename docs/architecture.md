...
消息提交后持久化运行任务。主任务全局顺序执行，每轮冻结实际使用的配置、资料和模型上下文。共享资料按稳定 ID 实时引用，资料编辑影响下一轮，不改写已保存的历史上下文。
...
正文、总结、变量变化、引用及回复建议在 SQLite 事务中原子提交。启用回复建议时，`rp_commit_turn` 先校验完整正文和 effects，再用本轮模型与冻结资料、拟提交后的变量生成结构化选项，最后将 `rp.reply-options` v1 extension 与正文一起写入唯一的 `turn.committed` 事件。生成请求期间不持有数据库事务，也不提前发布剧情；保存前重新检查上下文、故事状态和取消信号。关闭功能时跳过生成，运行中关闭也会取消选项请求并完成正文提交。
...
选项生成保留 30 秒超时且不自动重试。生成失败、超时或选项超出提交容量时，正文和状态仍可提交，原因随提交记录并在界面或执行记录中可查；取消整个运行则不提交正文、变量或选项。提交校验失败不会请求选项，重复成功提交不重复生成或应用 effects。删除、编辑、重生成和分支使建议随所属回复回放或失效。旧日志中的 `reply-options.ready` 仅用于读取历史数据，新生成流程不再写入独立选项或后台维护事件。父代理的完整模型消息用于恢复模型历史；请求副本与子代理内部日志用于诊断，都不代替剧情提交和变量事件。
