...
`install` 在已有数据时拒绝覆盖。首次安装如果在构建阶段失败，可用同一个域名和项目名重试；密码设置之后失败时使用 `up` 继续。`--pull` 要求独立、无未提交源码改动的 Git 克隆以及已配置上游的分支。命令在服务器上执行，不包含从开发机上传数据或推送镜像到公共仓库。
...
根目录的 [`.env.example`](../.env.example) 提供与初始化命令一致的默认值和字段说明，并随公开源码分发。首次部署先执行上述初始化命令，再按需编辑生成的 `.env`；仅复制示例文件不会创建所需的随机密钥和模型配置。本地开发入口不读取生产 `.env`。
